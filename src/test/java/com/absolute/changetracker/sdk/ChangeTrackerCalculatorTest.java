...
import com.absolute.changetracker.sdk.core.ChangeTrackerCalculator;
import com.absolute.changetracker.sdk.core.StandardChangeTrackerCalculator;
import com.absolute.changetracker.sdk.models.RowStatus;
import com.absolute.changetracker.sdk.testmodels.TestModel;
import com.absolute.changetracker.sdk.testmodels.TestModelItem;
...
class ChangeTrackerCalculatorTest {
    ...
    void diffBaseTest() {
        ...
        assertEquals(diffRow.getKey(), "BEFORE");
        assertNotNull(diffRow.getFields());
        assertNotNull(diffRow.getTables());
        assertEquals(diffRow.getFields().size(), 4);
        assertEquals(diffRow.getTables().size(), 0);
    }
    ...
    void diffWithsubModelsTest() {
        ...
        assertNotNull(diffRow.getFields());
        assertNotNull(diffRow.getTables());
        assertEquals(diffRow.getFields().size(), 0);
        assertEquals(diffRow.getTables().size(), 1);

        var table = diffRow.getTables().get(0);
        assertEquals(table.getName(), "rows");

        var tableRows = table.getRows();

        assertNotNull(tableRows);
        assertEquals(tableRows.size(), 3);
        assertTrue(tableRows.stream().anyMatch(el -> Objects.equals(el.getKey(), "row_1") && Objects.equals(el.getState(), RowStatus.Modified.toString())));
        assertTrue(tableRows.stream().anyMatch(el -> Objects.equals(el.getKey(), "row_2") && Objects.equals(el.getState(), RowStatus.Deleted.toString())));
        assertTrue(tableRows.stream().anyMatch(el -> Objects.equals(el.getKey(), "row_3_alt") && Objects.equals(el.getState(), RowStatus.New.toString())));
    }
